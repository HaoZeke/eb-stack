...
rung (§10.4): run it on the **site EasyBuild host**, not the laptop and not
the site cargo builder. Jenkins/site CI remains optional when the site
runbook says so.

### 11.5 Agent driver (campaign agent on the site EasyBuild host)

Drive this skill with a **campaign agent** (local-ai in herdr) **on the site
EasyBuild host**, not the laptop. Run `eb` / `eb-stack` there (robot tree +
modules). Prefer mechanical CLI steps; use the agent for residual judgment only
(same split as `skills/new-package/SKILL.md` §7). If the EasyBuild host is down,
stop and report.
