...
| `pypi_numpy/` | Warehouse JSON + robot + sdist tree | `numpy` already provided; leftover depends on `SciPy-bundle`; `torch` refuses or uses `PyTorch`; `eon-akmc-resolvo.json` plus `eon-akmc-resolvo/` wraps so SAT takes `PyYAML` / `quill` / `cbindgen` / `Eigen` from the robot |
