...
- ~generation_label~ :: Optional human label, e.g. ~"foss-2025b"~ when
  produced from an easyconfig tree. Library callers may omit it for hand-built
  universes.
...
[[file:policy.org][policy reference]]). This structure is a Rust/library input;
...
only the lock (and any requested build list / stack diff). ~eb-stack stack
sbom --lock LOCK~ defaults to ~stack.cdx.json~ because that command's sole
purpose is SBOM emission. Both paths now feed the same artifact facts: a
typed ~('sha256', …)~ source digest on the easyconfig becomes
~components[].hashes~ with ~alg: SHA-256~. Only the first checksum slot may
become that hash; a leading MD5 (or any non-SHA-256) is recorded as
~easybuild:checksum_unmapped~ and later 64-hex values as
~easybuild:checksum_extra~. Component identity includes versionsuffix as its
own ~+vs~ field, so a suffixed ~foss-2025b~ module and an unsuffixed
~foss-2025b-CUDA-12.8.0~ module cannot share a ~bom-ref~.
...
- From ~stack solve~ (universe in hand): each package's declared
...
- From ~eb-stack stack sbom --lock LOCK~ (lock only): ~dependsOn~ is empty
