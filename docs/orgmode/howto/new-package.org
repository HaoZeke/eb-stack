...
Missing robot dependencies are real package work. Close them with a layered
package-source catalog rather than inventing placeholders or dummy checksums.

* Catalog-backed robot holes

Pass one or more ~--package-catalog~ layers when ~package plan~ must recursively
close robot holes. Compatible robot candidates still win; the catalog is only
consulted for unsatisfied direct dependencies.
