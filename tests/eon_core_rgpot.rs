//! eOn 2.17.1 core + rgpot recipes under fixtures/eon_core_rgpot.
//!
...
fn eon_core_recipe_stays_conventional() {
    let text = std::fs::read_to_string(drafts().join("e/eOn/eOn-2.17.1-foss-2026.1.eb")).unwrap();
    for banned in [
        "preconfigopts",
        "postinstallcmds",
        "readcon-stage",
...
fn resolve_core_rgpot_companions() {
    let cases = [
        (
            "r/rgpot/rgpot-2.5.0-GCCcore-15.2.0.eb",
            "rgpot",
            "2.5.0",
            "GCCcore",
            "15.2.0",
...
fn eon_fat_2024a_and_core_2026_1_fixtures_coexist() {
    ...
    let a26 = drafts().join("e/eOn/eOn-2.17.1-foss-2026.1.eb");
