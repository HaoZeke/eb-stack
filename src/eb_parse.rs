...
/// One scaffolded companion easyconfig written for a missing robot dep.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ScaffoldedCompanion {
    pub path: String,
    pub name: String,
    pub version: String,
    pub toolchain: Toolchain,
...
pub fn companion_easyconfig_basename(
    name: &str,
    version: &str,
    tc: &Toolchain,
...
/// Render a minimal, parseable companion `.eb` for a missing dep.
///
/// The scaffold is intentionally honest: it has a placeholder homepage and a
/// dummy checksum so `packaging_gate` / robot identity matching work; humans
/// (or a follow-up fill-in pass) replace sources/checksums/configopts.
pub fn render_companion_scaffold(
    name: &str,
    version: &str,
    tc: &Toolchain,
    versionsuffix: Option<&str>,
    ...
    let vs = versionsuffix.unwrap_or("");
    let tc_line = if is_system_toolchain(tc) {
        "toolchain = SYSTEM".to_string()
    } else {
        format!(
    ...
    let vs_line = if vs.is_empty() {
        String::new()
    } else {
        format!("versionsuffix = '{vs}'\n")
    };
    // 64-char hex placeholder (not a real sha256 of a tarball).
    let dummy_sum = "0".repeat(64);
    format!(
        r#"# Scaffold generated by eb-stack check-recipe --scaffold-missing.
# Role: {role} dependency. Replace homepage/sources/checksums/configopts
# before production; keep name/version/toolchain identity stable for robot.
easyblock = 'CMakeMake'

name = '{name}'
version = '{version}'
{vs_line}homepage = 'https://example.invalid/TODO-{name}'
description = """Scaffold companion for {name}-{version}.
Fill in real sources, checksums, and build options before `eb` install."""

{tc_line}
toolchainopts = {{'pic': True}}

source_urls = ['https://example.invalid/TODO/']
...
/// Write scaffold `.eb` files under `out_root` (letter/name/ layout) for each
/// missing dep. Existing files are left untouched (`skipped_existing`).
pub fn scaffold_missing_companions(
    missing: &[MissingDep],
    out_root: &std::path::Path,
    default_toolchain: &Toolchain,
) -> Result<Vec<ScaffoldedCompanion>, String> {
    use std::fs;
    let mut out = Vec::new();
    for m in missing {
        let tc = m
            .toolchain
            .clone()
            .unwrap_or_else(|| default_toolchain.clone());
        ...
        if !skipped {
            let body = render_companion_scaffold(
                &m.name,
                &m.version,
                &tc,
...
"#;
    ...
    fn scaffold_missing_companions_writes_parseable_letter_layout() {
        ...
        let default_tc = Toolchain {
            name: "foss".into(),
            version: "2024a".into(),
        };
        let written =
            scaffold_missing_companions(&missing, dir.path(), &default_tc).expect("scaffold");
        assert_eq!(written.len(), 1);
        assert!(!written[0].skipped_existing);
        let p = std::path::Path::new(&written[0].path);
        assert!(p.is_file(), "{}", written[0].path);
        assert!(
            written[0].path.contains("m/metatomic-torch/"),
            "letter layout: {}",
            written[0].path
        );
        let r = resolve_easyconfig_file(p).expect("scaffold must parse");
        assert_eq!(r.name, "metatomic-torch");
        assert_eq!(r.version, "0.1.15");
        assert_eq!(r.toolchain.label(), "foss-2024a");
        assert_eq!(r.easyblock.as_deref(), Some("CMakeMake"));
        // Second pass skips existing.
        let again =
            scaffold_missing_companions(&missing, dir.path(), &default_tc).expect("re-scaffold");
        assert!(again[0].skipped_existing);
    }
