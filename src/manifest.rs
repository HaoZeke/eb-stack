...
use crate::foreign::{
    canonicalize_eb_package_name, emit_easyconfig_from_foreign, guess_easyblock,
    map_dep_name_to_eb_pub, residual_queue_from_ingest, ForeignDep, ForeignError, ForeignFormat,
    ForeignRecipe, ForeignRuleKind, ForeignSource, IngestOpts, IngestResult, ResidualClaimLadder,
};
use crate::hierarchy::{hierarchy_for_with_tree, resolve_dep_versions_for_specs, SourceDepSpec};
use crate::package::{
    BuildSpec as CanonicalBuildSpec, DependencyIntent, DependencyRole, OutputRequest,
    PackageMetadata, PackageOrigin, PackagePlan, PackageRule, PackageRuleKind, ProductProfile,
...
/// Convert a parsed foreign recipe into the version-one canonical package plan.
